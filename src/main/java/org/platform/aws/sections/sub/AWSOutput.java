...
public class AWSOutput {
    ...
    /** The Logical ID. */
    // An identifier for the current output. The logical ID must be alphanumeric
    // (a-z, A-Z, 0-9) and unique within the template.
    private String Logical_ID;

    // (optional)
    /** The Description. */
    // A String type that describes the output value. The value for the description
    // declaration must be a literal string that is between 0 and 1024 bytes in
    // length. You cannot use a parameter or function to specify the description.
